package com.jaylerrs.bikesquad.utility.sharedstring;

/**
 * Created by jaylerr on 14-Jul-17.
 */

public class FirebaseTag {

    public static String post_user =                    "posts-user";
    public static String post =                         "posts";
    public static String post_comments =                "post-comments";

    public static String users  =                       "users";
    public static String user_username =                "username";
    public static String user_email =                   "email";
    public static String user_birthDate =               "birthDate";
    public static String user_weight =                  "weight";
    public static String user_height =                  "height";
    public static String user_privacy =                 "privacy";

    public static String route =                        "route";
    public static String route_comments =               "route-comments";

    public static String route_name =                   "route-name";
    public static String route_detail =                 "route-detail";
    public static String route_lelel_dangerous =        "route-dangerous";
    public static String route_level_difficult =        "route-difficult";

    public static String star_count =                   "starCount";
}
